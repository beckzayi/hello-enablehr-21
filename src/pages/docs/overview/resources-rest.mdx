...
## Learn how to navigate the resources provided by the enablehr API.
...
This describes the resources that make up the official enablehr REST
API. If you have any problems or requests, please contact enablehr
Support or enablehr Premium Support.

### Current version

By default, all requests to https://api.enablehr.com receive the v3
version of the REST API. We encourage you to explicitly request this
version via the Accept header.

Accept: application/vnd.enablehr.v3+json

For information about enablehr's GraphQL API, see the v4
documentation. For information about migrating to GraphQL, see
"Migrating from REST."

### Schema

All API access is over HTTPS, and accessed from https://api.enablehr.com. All data is sent and received as JSON.

\$ curl -i https://api.enablehr.com/users/octocat/orgs
...
> X-enablehr-Media-Type: enablehr.v3
...
When you fetch a list of resources, the response includes a subset of
the attributes for that resource. This is the "summary" representation
of the resource. (Some attributes are computationally expensive for
the API to provide. For performance reasons, the summary
representation excludes those attributes. To obtain those attributes,
fetch the "detailed" representation.)

Example: When you get a list of repositories, you get the summary
representation of each repository. Here, we fetch the list of
repositories owned by the octokit organization:
...
all attributes for that resource. This is the "detailed"
representation of the resource. (Note that authorization sometimes
...
The documentation provides an example response for each API
method. The example response illustrates all attributes that are
returned by that method.

### Authentication

There are two ways to authenticate through enablehr REST API. Requests
...
$ curl -u "username" https://api.enablehr.com
OAuth2 token (sent in a header)
$ curl -H "Authorization: token OAUTH-TOKEN" https://api.enablehr.com
Note: enablehr recommends sending OAuth tokens using the Authorization header.
...
\$ curl -i https://api.enablehr.com -u foo:bar

> HTTP/1.1 401 Unauthorized

> {
> "message": "Bad credentials",
> "documentation_url": "https://developer.enablehr.com/v3"
> }
> After detecting several requests with invalid credentials within a short period, the API will temporarily reject all authentication attempts for that user (including ones with valid credentials) with 403 Forbidden:

\$ curl -i https://api.enablehr.com -u
...
\$ curl -i "https://api.enablehr.com/repos/vmg/redcarpet/issues?state=closed"
...
\$ curl -i -u username -d '{"scopes":["public_repo"]}' https://api.enablehr.com/authorizations
...
\$ curl
-u username:token https://api.enablehr.com
GraphQL global node IDs
...
\$ curl 'https://api.enablehr.com/user/repos?page=2&per_page=100'
...
Link: <https://api.enablehr.com/user/repos?page=3&per_page=100>; rel="next",
<https://api.enablehr.com/user/repos?page=50&per_page=100>; rel="last"
...
For users that belong to a enablehr Enterprise Cloud account, requests made using an OAuth token to resources owned by the same enablehr Enterprise Cloud account have an increased limit of 15,000 requests per hour.
...
\$ curl -i https://api.enablehr.com/users/octocat
...
> "documentation_url": "https://developer.enablehr.com/v3/#rate-limiting"
> }
...
Abuse rate limits
In order to provide quality service on enablehr, additional rate limits may apply to some actions when using the API. For example, using the API to rapidly create content, poll aggressively instead of using webhooks, make multiple concurrent requests, or repeatedly request data that is computationally expensive may result in abuse rate limiting.
...
> "documentation_url": "https://developer.enablehr.com/v3/#abuse-rate-limits"
> }

### User agent required

All API requests MUST include a valid User-Agent header. Requests with no User-Agent header will be rejected. We request that you use your enablehr username, or the name of your application, for the User-Agent header value. This allows us to contact you if there are problems.
...
\$ curl -iH 'User-Agent: ' https://api.enablehr.com/meta
...
\$ curl -i https://api.enablehr.com/user
...
\$ curl -i https://api.enablehr.com/user -H 'If-None-Match: "644b5b0155e6404a9cc4bd9d8b1ae730"'
...
\$ curl -i https://api.enablehr.com/user -H "If-Modified-Since: Thu, 05 Jul 2012 15:31:30 GMT"
...
\$ curl -i https://api.enablehr.com -H "Origin: http://example.com"
HTTP/1.1 302 Found
Access-Control-Allow-Origin: \*
Access-Control-Expose-Headers: ETag, Link, X-enablehr-OTP, X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset, X-OAuth-Scopes, X-Accepted-OAuth-Scopes, X-Poll-Interval

This is what the CORS preflight request looks like:

```
$ curl -i https://api.enablehr.com -H "Origin: http://example.com" -X OPTIONS
HTTP/1.1 204 No Content
Access-Control-Allow-Origin: *
Access-Control-Allow-Headers: Authorization, Content-Type, If-Match, If-Modified-Since, If-None-Match, If-Unmodified-Since, X-enablehr-OTP, X-Requested-With
...
to embed enablehr content in web pages by getting around cross domain
issues. The response includes the same data output as the regular API,
plus the relevant HTTP Header information.

\$ curl https://api.enablehr.com?callback=foo
...
> ["https://api.enablehr.com?page=2", {"rel": "next"}]
> ]
> },
> "data": {
> // the data
...
script.src = 'https://api.enablehr.com?callback=foo';
...
Some requests that create new data, such as creating a new commit,
allow you to provide time zone information when specifying or
generating timestamps. We apply the following rules, in order of
priority, to determine timezone information for API calls.
...
\$ curl -H "Time-Zone: Europe/Amsterdam" -X POST https://api.enablehr.com/repos/enablehr/linguist/contents/new_file.md
...
Using the last known timezone for the user
If no Time-Zone header is specified and you make an authenticated call to the API, we use the last known timezone for the authenticated user. The last known timezone is updated whenever you browse the enablehr website.
